    ...
    let handler = {
        ...
        move |req: Vec<u8>| {
            ...
            async move {
                ...
                oqueue_broker::HandlerResponse::Reply(req)
            }
        }
    };
...
async fn a_frame_over_the_cap_ends_the_connection_loudly() {
    let (mut client, server) = tokio::io::duplex(4096);
    let handler = |req: Vec<u8>| async move { oqueue_broker::HandlerResponse::Reply(req) };
    let conn = tokio::spawn(serve_connection(server, Arc::new(handler), LIMITS));
    client
...
async fn a_panicking_handler_is_this_connections_error_alone() {
    ...
    let handler = |_req: Vec<u8>| async move {
        ...
        {
            panic!("handler bug");
            oqueue_broker::HandlerResponse::Close
        }
    };
    let conn = tokio::spawn(serve_connection(server, Arc::new(handler), LIMITS));
...
async fn a_peer_that_never_reads_times_out_the_write() {
    ...
    let handler =
        |req: Vec<u8>| async move { oqueue_broker::HandlerResponse::Reply(req.repeat(16)) };
    let limits = ConnectionLimits {
        max_frame: 1024,
        max_in_flight: 2,
        idle_timeout: std::time::Duration::from_secs(30),
