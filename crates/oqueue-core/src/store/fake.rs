//! The in-memory [`ObjectStore`](super::ObjectStore) every test stands on.
//!
//! ⚠️ **Its own module because the contract and the double are two concepts**
//! (`code-structure.md` rule 18), and because `store.rs` reached the 500-line
//! limit holding both when `M5.6` added the streaming writer. The trait, its
//! documented guarantees and the streaming seam stay in `store.rs`; everything
...
/// The fake's streaming writer: parts accumulate, `finish` writes them whole.
///
/// ⚠️ **It buffers, and that is honest for a fake.** The property a real
/// backend's streaming writer buys is that the *process* need not hold the
/// object; the property this seam promises a caller is that it need not know
/// the size up front. A fake exists to hold an implementation to the second,
/// and `M5.6`'s own memory-bound test therefore counts what the *caller*
/// retains, not what the store does.
#[derive(Debug)]
pub struct FakeMultipartWriter<'store> {
    store: &'store FakeObjectStore,
    key: ObjectKey,
    parts: Vec<u8>,
    /// ⚠️ **Bytes, not "any part was written".** A real backend seals on
    /// whether anything was *uploaded*, so a stream of one empty part gets
    /// `EmptyBundle` there; a fake that counted calls instead would seal a
    /// zero-byte object and a test would pass here for a reason no backend
    /// shares — `contracts.md` rule 10's own hazard.
    bytes: usize,
    /// ⚠️ **A failed part poisons the writer.** The trait says a writer must
    /// not be used again after an error, and against a real backend the upload
    /// is already aborted, so a retried part would hit an upload that no longer
    /// exists. A fake that accepted it would seal an object missing the failed
    /// part.
    poisoned: bool,
}

impl<'store> MultipartWriter<'store> for FakeMultipartWriter<'store> {
    fn write_part(&mut self, part: Vec<u8>) -> BoxFuture<'_, Result<()>> {
        Box::pin(async move {
            if self.poisoned {
                return Err(Error::Permanent);
            }
            self.store.delay().await;
            if let Some(err) = self.store.take_storm_error() {
                self.poisoned = true;
                return Err(err);
            }
            self.bytes = self.bytes.saturating_add(part.len());
            self.parts.extend_from_slice(&part);
            Ok(())
        })
    }
    ...
    fn finish(self: Box<Self>) -> BoxFuture<'store, Result<ObjectMeta>> {
        Box::pin(async move {
            ...
            self.store.put(&self.key, self.parts, None).await
        })
    }
}
...
impl ObjectStore for FakeObjectStore {
    ...
    fn open_multipart<'a>(
        ...
        Box::pin(async move {
            ...
            let writer: Box<dyn MultipartWriter<'a> + 'a> = Box::new(FakeMultipartWriter {
                store: self,
                key: key.clone(),
                parts: Vec::new(),
                bytes: 0,
