...
- **Idempotence bootstrap**: `InitProducerId` (22) — many modern clients enable the idempotent producer path by default, which forces an `InitProducerId` call even when the application never asked for transactions. ⚠️ **CORRECTION (2026-09-02), from `M11.11`:** "librdkafka in particular" is wrong — a wire capture against confluent-kafka's librdkafka 2.15.0 found its own default for `enable.idempotence` is `false`, not `true`; only Kafka's Java client defaults it on, since KIP-679 (Kafka 3.0). The broader point stands (a client that opts in forces the call regardless of application intent), but the specific vendor claim does not.
