...
- **Idempotence bootstrap**: `InitProducerId` (22) — many modern clients (librdkafka in particular) enable the idempotent producer path by default, which forces an `InitProducerId` call even when the application never asked for transactions
