...
1. **Kafka's `Metadata` authorization-filtering behavior.** §4 asserts that unauthorized topics are excluded from the response and that Kafka's implementation filters O(all topics) per request. The protocol-level behavior is believed correct; the implementation claim is from general knowledge, **not re-verified in this corpus**. It is the load-bearing assumption of the entire design and should be checked against Kafka source before proceeding.
