...
| M10.31 | A pause that holds one store operation, not all of them | Serves NFR-20, NFR-21 — from `M10.10`, which measured the limit. ⚠️ **`FaultConfig::latency_polls` delays every `get`, `put` and `delete`**, so an observation taken while a PUT is pending has its own reads delayed too and the write lands inside them. `M10.10`'s in-flight look works only against an empty log, where FR-12 means the fetch issues no GET at all. ⚠️ **So `NFR-21`'s window can be entered exactly once per run, at the first produce**, and every later one is checked at rest. A per-operation pause — the shape `sim`'s `Fault::Pause` already has for the simulated S3 — is what would let the invariant be checked in the window it is about throughout a run. | todo |
