...
⚠️ **M3 carries 19 rows, one under `sdd.md`'s cap of 20.** Eighteen are the
decomposition `M3.0` derived from `M3.md`; the nineteenth (`M3.18`) is a sweep
row, the type `ADR-0016` created for review findings worth scheduling before
the milestone-boundary harvest. ⚠️ A twentieth would be the cap, so the next
finding worth a row is the one that has to argue for it.
