...
| M10.13 | The worked example: one known failure class, caught end to end | Serves NFR-20. ⚠️ **This is what makes the completion condition mean anything** — `M10.md` says so, and it is right: a harness that runs and finds nothing is indistinguishable from one that cannot find anything. | todo |
| M10.14 | `testing.md` says what the harness actually made possible | Serves no FR/NFR. The paragraph `M-1` deferred to the `M1.22` spike and `M1.22` deferred again to the harness itself. ⚠️ **Rule 26 currently reads "settled for object-store I/O and open for everything else"** — this row is what closes or re-states that, against what was built rather than what was hoped. | todo |
