...
| M2.11 | Sweep (from `M1.51`, `M1.55`): pinned-image agreement and the matrix-honesty check | A check compares each pinned image's two copies; `m1-complete.sh` performs the matrix-vs-existing-backends comparison its header claims, or the header stops claiming it; negative cases for both | todo |
| M2.12 | `oqueue-checksum`: CRC-32C via `crc-fast`, `crc32c_combine`, SIMD dispatch in a `OnceLock` | Serves FR-3. ⚠️ `crc32fast` is the wrong polynomial (doc 18 §4.3); dispatch per batch, env override. Kafka/RFC test vectors pass; differential test against a scalar reference | todo |
| M2.13 | `oqueue-codec`'s byte primitives for the frame and batch layers | Serves FR-1. Big-endian ints and bounded strings/bytes where the frame and RecordBatch paths need them — message-body primitives are `kafka-protocol`'s (ADR-0017). Round-trip property tests; every client-supplied length bounded before allocation (`security.md` rules 1-2) | todo |
