...
| M10.19 | Two refusals a client cannot act on | Serves FR-1, FR-2; `error-handling.md` rule 7. Received from `M3.43` (`dissolved`). ⚠️ **The one with a live client cost**: `RecordTooShort` reaches a producer as `CORRUPT_MESSAGE`, which the Java client and librdkafka both retry, so a client re-sends byte-identical bytes until `delivery.timeout.ms` expires. Also `CacheState::admits`'s check order, and `RecordTooShort`/`LengthOutOfBounds` both reporting `at: BATCH_HEADER_LEN`, the same byte in every batch. ⚠️ **The position must be taken *before* `read_varint`**: `M3.37` prescribed `BATCH_HEADER_LEN + cur.position()`, its review refuted it, and `M3.17` corrected the row — the cursor has already advanced past the varint when the error is built, so that expression names where the record *body* would start, off by the varint's width. | todo |
