...
| M1.44 | `ADR-0014` deferred GCS live verification to nobody | Serves NFR-40 — found by M1's milestone review. `GcsStore` ships in `M1.17` and **has never executed against anything**: the conformance matrix records `gcs` as not-yet-run, `ADR-0014` accepts that and defers verification, and no milestone owns the deferral. An accepted deferral with no receiving milestone is indistinguishable from an oversight three months later, which is exactly what `AGENTS.md` warns about two sections above the one describing the deferral table. ⚠️ The fix is a `roadmap.md` "Deferred into a later milestone" row plus a line in the receiving milestone's plan, not code — but note the shape of the risk: `s3.rs` and `gcs.rs` are ~90% duplicated, so `gcs` looks covered by `s3`'s green column while the 10% that differs (`ifGenerationMatch`, compose, resumable) is precisely the untested part | todo |
