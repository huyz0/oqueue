...
| M9.6 | `ApiVersions` answerable before authentication | Serves FR-4. Unchanged from `M2`'s own behaviour (doc 02 §1.4 point 2: sent "regardless of current authentication state") — verify the dispatch path still reaches it before `M9.3`'s handshake gates everything else, rather than assuming nothing in `M9.3`-`M9.5` moved it. | todo |
