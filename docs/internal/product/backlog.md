...
| M10.29 | `ADR-0005` says nothing about a cancelled `put` | Serves FR-10 — from `M10.8`'s review, which measured it. ⚠️ **Guarantee 2 is keyed on the future *resolving* `Err`**; `M10.8`'s pause drops it mid-flight with the object landed, which is a third state giving the caller strictly less than either clause. ⚠️ **`M10.8` is the first thing in the tree to produce it**, and calling it an instance of guarantee 2 — which that row's prose does — hides the gap: an audit of the commit protocol finds only the `Ok`/`Err` clauses and concludes the cancellation case is governed. `flush.rs`'s own doc already reasons about drops at two points, so the ADR is behind its callers. Decide whether the ADR gains a clause or the harness stops producing the state. | todo |
