...
| M2.4 | Decide (from `M1.54`): the out-of-bounds ranged-get contract | Start-past-size is `ByteRangeOutOfBounds` on the fake, `Transient` on both real backends. Contract settled — trait change, HEAD per ranged get, or a capability flag; the suite gains a case that sees the divergence | todo |
