...
2. `M10.2`, `M10.3`, `M10.4`, `M10.5` — the harness bottom-up: transport,
   model, scheduler, clock. Each is what the next has to run on. ⚠️ **`M10.24`
   belongs beside `M10.5`**: both are about what still reaches a real clock or
   a real machine outside the caps, and `M10.23` shipped the caps without the
   commit path inside them.
