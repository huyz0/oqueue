...
decomposition `M3.0` derived from `M3.md`. One (`M3.18`) is a **sweep** row,
the type `ADR-0016` created for review findings worth scheduling before the
milestone-boundary harvest. One (`M3.19`) is the checkpoint re-plan itself. And
**five** (`M3.20`-`M3.24`) come from M3's mid-milestone review: four are
`M3.14` split apart, because that row had accreted five independent
...
5. `M3.18`, the sweep — ⚠️ **before `M3.16`, not after it.** Its own row
   argues the `find_batches` benchmark cannot wait: `check-hot-path-bench.sh`'s
   allowlist entry is excused by "M3 has not landed", and M3's close falsifies
   that reason while the script's staleness check cannot fire, so the row
   would go uncovered permanently. It also carries `M3.10`'s major, which is a
   correctness fix rather than prose.
6. **`M3.16`**, which cannot assert its zero-GET and bounded-GET claims until
   `M3.20` and `M3.22` exist.
7. **`M3.17`** last, because a closing milestone review over unfinished rows
...
| M3.18 | Sweep (from `M3.7`'s review): a fault decorator beside `MetadataLog`, the untested overflow guards, and group commit | Serves FR-10, FR-11, NFR-1. Three findings recorded in `M3.7`'s commit body rather than fixed (`review.md` rule 15). ⚠️ **A decorator beside the trait in `oqueue-core`, not a third hand-written stand-in** — `contracts.md` rule 9, and `FaultConfig`/`CountingObjectStore` beside `ObjectStore` are the shape; `oqueue-coordinator`'s test-local `RefusableLog` is deleted in favour of it, and `M3.8`'s `CatchUp.reads` stops being the only way to see a log round trip. ⚠️ **It is also what lets `ADR-0024`'s serialization property be *asserted* rather than argued**: a decorator that can hold an `append` open is what a test needs to race a `drop_cache` against a commit already inside `serve`, and today an implementation clearing the index straight from a handle would pass the suite unchanged. `CoordinatorError::Unassignable`'s two overflow guards become assertable — a `cfg(test)` seeding constructor on `Allocator`, since reaching them through `commit` needs ~2^31 commits. And `CoordinatorLoop::run` drains up to N queued requests into **one** `append`: `MetadataLog::append` already takes a slice, and without it a durable engine at single-digit-millisecond commits turns `COMMIT_QUEUE_DEPTH` into seconds of latency against NFR-1's 500 ms p99. ⚠️ **Plus two from `M3.8`'s review.** `IndexState::find_batches` is `performance.md` rule 18's *offset→object index lookup* — the rule says the benchmark arrives *with* the code, `M3.8` is the commit that added the lookup, and `check-hot-path-bench.sh`'s `NOT_YET_BUILT` entry keeps the gate green on a reason (`M3 has not landed`) that M3's own close falsifies while the script's staleness check cannot fire, so the row would go uncovered permanently. And `MaterializedIndex` guarantee 2's **staged-entry** half is pinned by nothing now that `find_batches` makes it a one-line assertion: an eager push into `IndexState`'s partitions would leave an `ObjectRef` behind a refused batch and hand a fetch at the high watermark an object past it. ⚠️ **Plus one from `M3.9`'s.** `CoordinatorLoop::rebuild` is an unbounded full-log replay awaited **inside `serve`**, before the ack and inside the single serialization point, so a dropped cache on a shard with a long log blocks every queued producer for the length of the replay against NFR-1's 500 ms p99 — and a *failed* rebuild leaves `applied_upto` at `None`, so the next commit replays again, and so on until one succeeds. `M6.md` task 15 (chunked replay, bounded transaction size, periodic durable progress) is the mechanism; what this row owns is not paying for it on the ack path. ⚠️ **Plus `M3.10`'s major**: `RefreshReason::WatermarkFromAnotherEpoch` fires on inequality and documents the cache as "current and correct", which holds only when the watermark is *older* — a watermark from a **newer** epoch is the one piece of evidence a reader-side cache has that its own coordinator is the departed one, and the documented remediation sends the session back to it. `CoordinatorEpoch` is `Ord`, so the direction is a comparison; `ADR-0023` needs the same correction. ⚠️ **And a gate green on reasons already false** (M3's checkpoint review): `check-hot-path-bench.sh`'s `NOT_YET_BUILT` excuses three rows with "M2 has not landed" while `roadmap.md` marks M2 complete, and the script's own stale-entry check cannot fire until someone does the work the entry excuses — so `performance.md` rule 19's "the list above cannot silently rot" is held by nothing. Fix the reasons, and give the gate a way to fail on one that has expired. ⚠️ **And the rest of the harvest that finding names**, which is the point of a sweep row: `commit_version.rs` still teaches "`ReadMode::AtLeast(v)` is a `u64` compare and nothing more", which `read_mode.rs` and `ADR-0023` made false; `IndexState::tail()` returns an owned `Vec`, deep-cloning up to `TAIL_WINDOW_ENTRIES` entries, with no caller and in the type whose own comment refuses a tuple key to avoid one clone per lookup; and the paragraph duplicated verbatim at `index_state.rs`'s module doc and again above `IndexState` — the `M3.6` minor that the `M3.8` module split relocated rather than resolved. ⚠️ **And `M3.10`'s last two**, which are the ones with teeth: `CacheState::admits` runs the silence breaker *before* the mode match, so a `Linearizable` `ListOffsets` on a silent agent reports `PushStreamSilent` — a cache fault, at the `ListOffsets` rate, for a read that was never going to touch the cache — and `RefreshReason::Authoritative` reads zero exactly when an operator is diagnosing; and **no test builds a `CacheState` with `applied: None`**, so flipping `is_some_and` to `is_none_or` leaves the suite green while an index that has folded nothing admits an `AtLeast(v)` read and answers a producer's own consumer with an empty partition. ⚠️ **And `M3.13`'s major**: `parse_footer` is a decoder over stored bytes — `count`, `footer_len`, `name_len` and two `u64` ranges all come off the object — so `security.md` rule 5 and `testing.md` rule 24 owe it a fuzz target, and it has none. ⚠️ **The gate cannot say so either**: `scripts/fuzz.sh` walks `crates/oqueue-codec/src/*.rs` only, so a parser in `oqueue-core` is invisible to the check that fails a decoder module with no target. Both halves are this row's — the target, and the gate learning where to look — because a target nothing requires is one a later rename silently deletes. ⚠️ **Plus `M3.13`'s two recorded majors** (`review.md` rule 15, its commit body): `bundle_footer.rs`'s `check_regions` requires regions ascending and disjoint but **not contiguous from zero**, so shortening one region's length field still parses and a reader serves a truncated batch as that topic's data — `BundleBuilder::push` always sets `offset = payload.len()`, so `bounds.offset() == next_free` plus `next_free == payload_end` closes it in two comparisons; and a tail too short to hold the footer returns `MalformedBundleFooter`, which `retry_class` makes `RetryClass::Never`, so a healthy object read with an under-sized tail GET is indistinguishable from permanent corruption when the answer is to re-GET `footer_len + TRAILER_LEN` — a number `parse_footer` computed and discarded. ⚠️ **Plus `M3.14`'s six recorded minors**, five of which are documents that commit itself falsified: `store.rs`'s `set_faults` carrying `fn faults()`'s rustdoc; `ingest.rs`'s `Verified::records` still saying "floored at one" twenty lines above the refusal that replaced the flooring; `bundle_name.rs` still saying nothing mints a writer identity; `bin/oqueue`'s crate doc still saying "over a stub partition" where its own README no longer does; `metadata.rs` still saying "against the stub cluster", with three comments pointing at the deleted `produce.rs`. ⚠️ **The sixth is not prose**: `records::count_records` counts a zero-length record as a record, so N `0x00` bytes walk as N records and `verify` accepts them — one byte per offset rather than the unbounded lie the count check closed, and a minimum per-record length ends it. ⚠️ **Plus one `M3.21` found in code it did not write.** `Fetch` below v13 has the same null-name defect `M3.21` fixed for `ListOffsets`: `oqueue-codec`'s decoder reads the topic name as nullable, the response field is *not* nullable, and a request carrying a null frames a reply no client can parse — the Java client throws in its response parser and librdkafka reports a protocol read error. ⚠️ **The sibling finding — `fetch/partition.rs`'s read-failure path answering `LEADER_NOT_AVAILABLE`, which the Java consumer turns into an `IllegalStateException` — is already fixed**, by `M3.22`, which rewrote that arm; it is struck here rather than left to be discovered done. ⚠️ **Plus `M3.13`'s two recorded minors**, which the `M3.15` review found missing from this harvest: `oqueue-store/tests/it/retry.rs` has a doc comment saying "three bounded retries" three lines above an assertion that says two — the same attempts-versus-retries confusion that produced `M3.13`'s blocking finding, surviving in that fix's own test; and `BundleNamer::new` rejects an empty writer identity as `EmptyObjectKey`, whose `Display` names a resource that was not the one refused, while the sibling `/` case one branch later has a purpose-built `MalformedWriterId` | todo |
