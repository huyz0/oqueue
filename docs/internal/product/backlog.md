...
| M11.10 | Conformance: librdkafka with `enable.idempotence=true`, end to end | Serves FR-14; `testing.md` rule 20a. A client conformance test with idempotence on (librdkafka's default) produces successfully; a duplicate-sequence test observes deduplication (the exact-replay case answers the original offset, not an error); a fenced epoch is refused. Doc 06 §6 flags documented librdkafka compatibility gaps for the *retroactive* mechanism specifically — `ADR-0031` does not use that mechanism, but this is still where any gap this design does have would surface, not the unit tests. | todo |
| M11.11 | `scripts/gates/m11-complete.sh`, the milestone's own completion gate | Serves FR-14 (the method). Asserts the conformance case (`M11.10`), the fencing test (`M11.7`), and that M2's workaround is gone (`M11.9`) — the three legs `M11.md`'s own completion condition names. Each leg falsifiable or says it is not, on `M3.16`/`M10.15`'s precedent. | todo |
...
mechanism already built, order among themselves does not matter; `M11.10`-`M11.11`
close the milestone, in that order.
