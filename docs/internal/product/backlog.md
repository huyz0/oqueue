...
⚠️ **M0 carries 22 rows, over `sdd.md`'s cap of 20, and here is the argument the
...
rather than new scope smuggled in. The two rows past 20 are not scope either —
`M0.20` acts on the checkpoint review and `M0.21` is work split *out* of
`M0.8` because that row had grown too large for one commit. ⚠️ The forcing
...
| M0.21 | The three manifest assertions in `check-layering.sh`, split out of `M0.8` | The gate rejects a crate manifest without `lints.workspace = true`, a **member** manifest carrying a `[profile]` section, and a root manifest whose `[profile.release]` lacks `overflow-checks = true` — ⚠️ the last is named by `security.md` rule 4 ("→ gate on the profile setting") and exists nowhere; each gets a `tests/gates/negative.sh` case; ⚠️ `check-layering.sh`'s header, its `name:` in `.pre-commit-config.yaml`, and its row in the standards tables are updated to say it checks manifests rather than layering alone, or `security.md` rule 4's gate ends up under a name nobody would grep for. ⚠️ Names no FR/NFR — it is gate work split out of `M0.8`, and `M0.0`'s criterion that every other row names one was true when `M0.0` closed | todo |
