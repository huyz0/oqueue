...
| M11.13 | Record M11's closing milestone-review findings | Serves no FR/NFR — the closing bookkeeping `M10.16`'s own precedent names. Fresh cross-cutting reviewer covers the whole milestone (14 commits: `M11.0`-`M11.12`, plus the standalone `M11.1` fuzz-gap fix); findings fixed or argued. ⚠️ Watch for `M10.16`'s own discovered failure mode: a closing commit that touches anything outside `reviews/` is an uncovered commit of its own unless it is the *last* commit, and a lettered or otherwise irregularly-named task id being silently dropped from coverage. | todo |
