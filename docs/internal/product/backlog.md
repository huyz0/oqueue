...
2. `M3.23`, because its ADR and the read-only view are what make the
   sole-writer rule type-checked, and `M3.11`'s degraded mode is the first
   code that would otherwise be written against a handle still carrying
   `clear`.
...
| M3.11 | Index growth: enforced, alarmed quota with an explicit degraded mode | Serves NFR-11 (per-node metadata cost proportional to partitions active on that node). Not a dashboard (task 18); cleanup on by default. Test: exceeding the quota trips the degraded mode observably. ⚠️ **`M3.24` settled where this lives: `MemoryIndex`, in `oqueue-index`.** Not `IndexState` in `oqueue-core` — that fold is shared with the fake beside the trait, and a quota is the first thing a broker's materialization has that a downstream crate's test double must not. This row is what makes the two stop being byte-identical. ⚠️ **The degraded mode asks the coordinator to drop the cache; it must not reach for the handle** (`M3.9`): the coordinator is the sole writer of its own index and the only party that may clear it, because `MaterializedIndex::apply` checks version order and *not* contiguity — a `clear` landing between the coordinator's own check and its fold is accepted and re-bases every partition at `Offset::ZERO`, which is an index that is wrong rather than one that is empty | todo |
