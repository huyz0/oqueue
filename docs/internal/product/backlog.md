...
5. `M10.15`, the completion condition — after the catch, which is what makes it
   mean anything.
6. `M10.34`, `M10.35` before `M10.16` — each a finding the closing review
   itself made, and `M10.16`'s own row says a closing review over unfinished
   rows is the one thing it must not be.
