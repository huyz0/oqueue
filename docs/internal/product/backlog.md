...
| M10.21 | The charging model, in every site that still denies it | Serves FR-13; `behavior.md`. Received from `M3.45` (`dissolved`). ⚠️ **Three are in `docs/protocol-support.md`, the published FR-2 contract**, and one of the three is falsified by a test M3 itself committed; five are rustdoc — `read_or_refresh`, `MAX_FAILED_FETCHES_PER_REQUEST`'s rationale, `FetchedObjects`' own doc, `read.rs`'s tail-read paragraph, and `tests/it/reads.rs`'s module doc. ⚠️ **The criterion is a search, not a count, and this row is the third time that lesson has been paid for**: `M3.45` named four rustdoc sites and gained a fifth on review; the review of `M10.0` then found a sixth, `tests/it/faults.rs`'s module doc, which the same file's own `a_failed_parse_leaves_the_exemption_for_the_partition_behind_it` contradicts. A fixed count closes the row with one still standing — which is what `M3.37` did to `read.rs:90`. Done means no site in the tree denies the model, established by searching for the claim rather than by working a list. | todo |
