...
| M10.20 | The two files at the 500-line limit | Serves no FR/NFR; `code-structure.md`'s limit as a design signal. Received from `M3.44` (`dissolved`): `index_state.rs` is `ADR-0022`'s fold and its read surface, `records.rs` is `RecordIter` and `count_records`, and each split must carry what the row names — `Page`'s contiguity rule not becoming doc-only, and a fuzz target for `count_records` rather than an exemption. | todo |
