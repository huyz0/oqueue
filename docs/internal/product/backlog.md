...
8. `M3.35`, `M3.36` — the gates and the bounds, after the constants they pin
   stop moving.
9. `M3.38`, after every behaviour change above, so the published documents
   describe what shipped rather than what was intended.
10. `M3.39`, which is a behaviour change rather than a bound, and needs
   `M3.26`'s charge to have settled first.
