...
| M10.25 | NFR-56's budget was written for a native suite, and the commit path is no longer one | Serves NFR-56. From `M10.24`'s review. ⚠️ **Containment roughly doubled the measured suite**, and these are the min and max of every 17-gate `ok` row in each `suite.tsv` rather than a pair of representative runs — the distinction matters because this row is the record someone will set a number from. Host, native, n=74: **3510-6305 ms**. Container, n=12: **7658-9456 ms**. Against a `BUDGET_MS` of 10000 that non-negotiable 2 forbids raising, so the worst contained run is at **94.6%** of the ceiling and the native distribution already reaches 63% of it. The first busier host, lower `CPUS`, or added dependency refuses a commit, and the only exit is `OQUEUE_NO_CONTAINER=1`, which undoes the containment. ⚠️ **And ~2 s of it is invisible to the gate**: the `docker info` probe, the stale-image scan and the volume marker checks are wall clock NFR-56 is written to include and `check-budget.sh` structurally cannot count — measured 11.3 s wall for a run that counted 9350 ms. ⚠️ **This is a decision, not a patch**: either NFR-56's number is about a native suite and the contained path needs its own stated budget, or the requirement's wall-clock reading stands and containment has to get cheaper. Raising the constant is the one answer non-negotiable 2 rules out. | todo |
