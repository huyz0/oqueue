...
| M10.6 | Latency from doc 04 §2's distributions, not from round numbers | Serves NFR-20. ⚠️ **Round numbers are the failure mode this row exists for**: a p50 that is always the p50 hides every schedule where the tail is what breaks the system, and doc 04 §2 is where the real shape is already measured. | todo |
| M10.7 | Fault injection: 503 storms, conditional-write races, partial failures | Serves FR-51 (the method), NFR-20. The framework plus the three faults M3 already has real code paths for — `oqueue-store`'s `classify.rs` and `retry.rs`, and the coordinator's refused journal. | todo |
