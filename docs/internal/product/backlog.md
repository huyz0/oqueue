...
| M10.17 | A response name that cannot be null, in the type | Serves FR-1, FR-2. Received from `M3.41` (`dissolved`), which carries the finding and the shape decision: `ListOffsets`' response name can be `&'a str` because no `None` is legal at any advertised version, while `Produce` and `Fetch` need a name-or-id sum type because above v13 there genuinely is no name. The codec half is pinned by nothing today. | todo |
