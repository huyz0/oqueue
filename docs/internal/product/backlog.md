...
| M2.14 | Varint/zigzag with a SWAR decoder | Serves FR-3. ⚠️ Not `varint-simd` — it reads past the varint on untrusted input. Round-trip property tests plus boundary vectors; a fuzz target registered for `M2.26` | todo |
| M2.15 | The advertised-versions table, pinning `kafka-protocol`'s flexible encoding | Serves FR-1, FR-2. The flexible rules are the dependency's (ADR-0017); ours is the per-(API, version) table of what this broker advertises, plus golden tests pinning the crate's encoding at each advertised version — ⚠️ no global cutover: Produce v9, Fetch v12, ApiVersions v3 | todo |
| M2.16 | Frame codec, and headers via `kafka-protocol`, with the `ApiVersions` header special case | Serves FR-1. The frame codec is ours; header types are the dependency's (ADR-0017). ⚠️ `ApiVersions` responses carry a v0 header even when the body is flexible. Golden bytes pin the special case; correlation-ID extraction tested | todo |
| M2.17 | Connection task: read/write split, pipelining, out-of-order responses, bounded backpressure | Serves FR-1; `async-concurrency.md` applies throughout. A pipelined test observes out-of-order completion with per-connection in-order writes; channels bounded | todo |
| M2.18 | RecordBatch v2 header and attributes bitfield | Serves FR-3. The 61-byte fixed layout; `magic != 2` rejected. A real client's golden batch decodes; every attributes field round-trips | todo |
| M2.19 | CRC verify on ingest and the header-only offset rewrite | Serves FR-3. ⚠️ Offset assignment decodes zero varints (doc 18 §4.4). Differential CRC test; the rewrite leaves payload bytes untouched, byte-compared | todo |
| M2.20 | Record-level iterator (opt-in) and the compression seam | Serves FR-3. none/lz4/zstd wired, gzip/snappy behind features; ⚠️ not on the produce/fetch path. Iterator tests per codec; the hot path shown not to iterate | todo |
| M2.21 | `ApiVersions` v0-v3 with the `UNSUPPORTED_VERSION` v0-bodied fallback | Serves FR-2. ⚠️ Answered before authentication. A v99 request gets the v0-bodied fallback; a real client negotiates | todo |
| M2.22 | `Metadata`, honouring `AllowAutoTopicCreation` from v4 | Serves FR-1. ⚠️ Broker identity (node ID, hostname) chosen deliberately, not by convenience. A real client bootstraps against the stub | todo |
| M2.23 | `Produce` v3-13 against a stub partition | Serves FR-1. v0-2 removed by KIP-896. librdkafka's produce round trip lands records in the stub | todo |
| M2.24 | `Fetch` v4-17, echoing session fields, parsing `IsolationLevel` | Serves FR-1. librdkafka's fetch returns the records `M2.23` produced | todo |
| M2.25 | Golden-byte corpus, librdkafka harness, protocol-support matrix, `scripts/gates/m2-complete.sh` | Serves FR-1, FR-2. ⚠️ Decide the idempotence question explicitly — stub `InitProducerId` or document `enable.idempotence=false`. The gate asserts the completion condition: client round trips, byte-exact re-encode, CRC differential | todo |
