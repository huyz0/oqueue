...
| M2.5 | Decide (from `M1.40`): should a persistently `compiling` suite fail rather than skip | Decision recorded; `check-budget.sh`'s exemption branch and its warning match it | todo |
| M2.6 | Decide (from `M1.41`): should a standard select itself for review | Decision recorded; `applies_to` conventions updated to match, or declined with the reason stated | todo |
| M2.7 | Decide (from `M1.49`): non-negotiable 2 names the wrong direction for reciprocal thresholds | `AGENTS.md` rule 2 and `check-drift.sh`'s header agree with `m0-complete.sh`'s per-entry direction table | todo |
| M2.8 | Shape, then fix (from `M1.48`): two sessions sharing one git index went undetected | Its row rejects the `target/` lockfile. A chosen mechanism (candidate: `review.sh` records session identity beside the diff hash) lets `check-reviewed.sh` say *why* staged bytes moved; the four carried minors addressed or re-recorded | todo |
| M2.9 | Shape, then fix (from `M1.50`): five `clippy.toml` thresholds pinned by nothing on the commit path | `NFR_CONSTANTS` greps `NAME=` and cannot read TOML. A mechanism pins them; a negative case fails on a raised threshold | todo |
| M2.10 | Sweep (from `M1.47`, `M1.56`): the four stale ADRs, including ADR-0008's unwired retry premise | ADR-0005/0008/0010/0013 each corrected via a dated status note or the code wired to match; `M1.56`'s wire-or-record choice made explicitly | todo |
