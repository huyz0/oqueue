...
| M1.46 | `check-milestone-review.sh` reports a milestone reviewed when it enumerated nothing | Serves no FR/NFR — found by `M1.43`'s review. The script `skip`s and exits **0** when the milestone has no commits, so its caller prints `ok every M1 commit is covered by a milestone review` having verified nothing. ⚠️ **That is the same skip-is-a-pass shape `M1.43` had just closed one level up**, which is what makes it worth a row rather than a note: the fix moved the check above every `skip` in `m1-complete.sh`, and the hole simply moved inside the script being called. Reproduced: a repo whose only M1-named commit touches solely `reviews/milestone-M1-*.json` makes the enumeration empty — that commit is deliberately excluded by `milestone_commits()`, the exclusion that stops the review regress — and the pair `skip milestone review (M1 has no commits yet)` / `ok every M1 commit is covered` prints back to back. ⚠️ Realistic trigger named by review: a shallow checkout (`actions/checkout` defaults to `fetch-depth: 1`) at the moment the verdict commit is the tip, which is precisely when a milestone closes. ⚠️ Decide which it is before patching — "no commits" is legitimately true early in a milestone, so a bare `fail` would make the gate unpassable at the start; the shape that fits is probably distinguishing *no history* (shallow clone, a real problem) from *no commits yet* (fine), the same distinction `M1.43` drew between exit 127 and exit 1 | todo |
