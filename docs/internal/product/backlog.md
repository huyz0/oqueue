...
⚠️ **M3 is close to `sdd.md`'s cap of 20 rows**, and the count is deliberately
not written here — `grep -c '^| M3\.' docs/internal/product/backlog.md`
derives it, and `sdd.md` says no count a command can derive goes into prose.
Most rows are the decomposition `M3.0` derived from `M3.md`; the rest are
**sweep** rows, the type `ADR-0016` created for review findings worth
scheduling before the milestone-boundary harvest. The next row added is the one
that has to check the count and argue for itself if it is the twenty-first.
...
| M3.14 | Wire Produce/Fetch to the real coordinator, replacing M2's stub partition | Serves FR-1, FR-10, FR-11, FR-12, NFR-2. `oqueue-broker`'s handlers compose `oqueue-coordinator`/`oqueue-index`/`oqueue-store` instead of M2's stub. A real client's produce-then-fetch round trip is durable and ordered end to end. ⚠️ **Receives `M3.9`'s wire-level half** (task 17): `max_wait_ms` stops being parsed-and-discarded and becomes a `select!` between `IndexWatch::wait_for` and the request's own deadline — no poll interval, and the end-to-end "a fetch racing a concurrent produce sees it" test lands here, because this is the first commit with a composer that can hold both sides. ⚠️ **And `M3.10`'s wire half**: `ListOffsets(LATEST/EARLIEST)` routed `Linearizable` to the coordinator and never derived from cache (H1); the session carrying `CommitAck::watermark()` across requests so a consumer's fetch is `AtLeast(v_produced)` (H2); and a 404 from the store meaning *reaped* rather than *not yet written* — object ids are never reused — retried through a coordinator refresh before `OFFSET_OUT_OF_RANGE`, instrumented, because a nonzero rate means `M5`'s deletion delay is too short (H4's second half). ⚠️ Also where `ADR-0022`'s overshoot bound stops being theoretical: `find_batches` bounds what it can price and **this handler is the real budget**, stopping when its own `max_bytes` fills as it learns each object's size | todo |
