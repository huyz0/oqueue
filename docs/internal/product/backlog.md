...
| M10.14 | `testing.md` says what the harness actually made possible | Serves no FR/NFR. ⚠️ **And carries two obligations from `ADR-0028`**: that a recorded seed means one schedule only against the toolchain and `tokio` version it was recorded under — `RngSeed::from_bytes` hashes with `DefaultHasher`, whose output std documents as unstable — and that `--cfg tokio_unstable` compiles scheduler poll hooks into the *release* binary, unmeasured. ⚠️ **And says it once, for all three pieces**: `ADR-0027` splits the harness across `oqueue-testkit`, `oqueue-store` and `oqueue-broker`'s test tree, which is a real cost — a reader looking for "the simulation" finds a third of it — and this row is the single place the shape gets stated. The paragraph `M-1` deferred to the `M1.22` spike and `M1.22` deferred again to the harness itself. ⚠️ **Rule 26 currently reads "settled for object-store I/O and open for everything else"** — this row is what closes or re-states that, against what was built rather than what was hoped. | todo |
