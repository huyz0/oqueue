...
| M10.27 | Sweep: the clock gate's fixtures discriminate, and its scope is the crate's test tree | Serves no FR/NFR — three findings from `M10.6`'s review, one theme, one commit (`sdd.md`'s sweep). ⚠️ **The major is that six cases pin nothing they claim to**: the mixed-line fixture also plants a `src/lib.rs`, so a `scan_clock` reverted to a line filter still exits non-zero with the expected text and the whole clock set stays green — measured by the reviewer against the mutant. Deleting the redundant file is the fix, and `src/` already has its own case. ⚠️ **And the licence is wider than the rule it serves**: `*/tests/*` matches any path component named `tests`, so `crates/*/src/tests/` — compiled into the shipped library unless `#[cfg(test)]`-gated — takes the runtime-clock exemption; anchoring to the crate root closes it. ⚠️ **While being narrower than the idiom**: only the fully-qualified `tokio::time::Instant::now()` is deleted, so `use tokio::time::Instant;` plus `Instant::now()` in a test tree fails under a remedy ("inject `Clock`") that does not apply to it. | todo |
