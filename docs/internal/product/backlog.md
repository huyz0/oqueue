...
| M10.30 | A fifth cause the crash-point enumeration does not name | Serves FR-10, NFR-20 — from `M10.9`'s round-3 review. ⚠️ **The broker dying mid-flight in the middle window matches no variant**: `{error_code: None, object_written: true, position_journalled: false, records_readable: false}` is neither of the two middle causes (both answer a code) nor `AfterCommitBeforeAck` (which is journalled). ⚠️ **And `M10.29` says the tree already produces it** — `M10.8`'s pause drops a request future with the object landed — so this is not hypothetical. `crash_points.rs`'s hedge is scoped to a fourth `.await` appearing in `flush` and does not cover a third *cause* in an existing window, which matters because `M10.15`'s leg is routed to build on that enumeration. | todo |
