...
| M10.13 | The worked example: one known failure class, caught end to end | Serves NFR-20. ⚠️ **`bin/oqueue`'s accept loop is out of reach of a *simulated* run** (`ADR-0027`): it has no `[lib]`, so the `select!` racing the coordinator's `serving` handle against `listener.accept()` — whose own comment says a failure there answers every produce `LEADER_NOT_AVAILABLE` "while looking healthy" — is driven by no simulation. ⚠️ **Take the cheap half first**: `serve.rs:238` already has a `#[cfg(test)] mod tests` that can construct `accept_loop`'s arguments and assert the `joined` arm, no crate change needed. Extracting a lib is what simulating it would cost, and this row is where that cost becomes visible if it is still worth paying. ⚠️ **This is what makes the completion condition mean anything** — `M10.md` says so, and it is right: a harness that runs and finds nothing is indistinguishable from one that cannot find anything. | todo |
