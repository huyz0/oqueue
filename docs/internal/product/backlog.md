...
| M9.17 | Cost test for NFR-12: `Metadata` response size and CPU independent of catalog size | Serves NFR-12, the acceptance criterion `m9-complete.sh` asserts directly. Measured at two catalog sizes an order of magnitude apart — a single size cannot distinguish O(1) from O(n) with a large constant. | todo |
