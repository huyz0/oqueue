...
| M10.32 | A workload where `park.rs`'s `select!` is genuinely contended | Serves NFR-20 — from `M10.12`, which measured the gap and named it rather than implying it closed. ⚠️ **A seed choosing a workload is not the same as a branch choice being observable.** `M10.11` moved the run through `run_seeded`, and review found it made zero draws from the seeded RNG; `M10.12` gave the seed a `Schedule` to draw, so two seeds now produce different `orphans`/`checks` — but `park.rs`'s two `select!` arms are still never ready *together* in this workload, because a single produce's commit always wakes the watch before the paused clock has any reason to advance to the parked fetch's deadline. So the branch order still changes nothing observable, and every seed's `the_run_consumes_the_seeded_rng` measurement pins a position on a choice rather than a divergence. ⚠️ **What would close it is genuine contention** — two or more produces racing one parked fetch, so which commit the watch sees first is a real question — which needs `RacedProduce` steps that overlap rather than serialize. | todo |
