...
| M10.18b | `check-hot-path-bench.sh`'s six unpinned branches | Serves NFR-3; `testing.md` rule 20a. Received from `M3.42` (`dissolved`), third of three. Six of its seven `M3.36` branches have no negative case, so each is a gate leg watched to pass and never watched to fail. | todo |
