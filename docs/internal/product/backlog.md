...
| M10.28 | Decide what a per-key delete refusal classifies as | Serves FR-51 (the method) — from `M10.7`, which measured it. ⚠️ **`object_store` publishes no shape for this failure at all**, which is what makes the question harder than picking an arm. Its public `Error` has twelve variants and none of them is a per-key delete failure; `aws/client.rs` builds a crate-private `DeleteFailed` carrying the key, the code and the message, and converts all but two variants of that enum to `Generic` with the original boxed as an opaque source — so the outcome is `Transient` and the information a decision would need has been erased rather than mismatched. ⚠️ **The consequence is real in both directions**: an `AccessDenied` hold no retry can fix is retried, bounded, by every caller obeying `retry_class()`, and an `InternalError` — which S3 also returns per key — would be right to retry. So the question is whether the code S3 sent can be recovered at all — `error-handling.md` rule 8 bans reading it out of a `to_string()`, and there is no typed route left — and what a GC pass should do with a key it may never delete. ⚠️ **An upstream change may be the only honest answer**, which is a different kind of task from the one this row's title suggests. `M10.7` pins today's answer, so a change here is a decision rather than a discovery. | todo |
