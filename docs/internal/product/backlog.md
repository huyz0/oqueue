...
| M2.3 | Decide (from `M1.53`): is the conditional-write ceiling a separate limit from the part size | A conditional write over 8 MiB is `Ok` on S3 and `Err(Permanent)` on GCS. Decision recorded; the divergence closed or a `Capabilities` flag carries it; a conformance case sees it | todo |
