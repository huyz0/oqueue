...
⚠️ **M1 carries 39 rows, over `sdd.md`'s cap of 20, and here is the argument
...
`M1.33` through `M1.38` are findings review made after this
...
label pinned by accident, a crate documenting a backend it does not have, and a universal claim that outlived its own correction — so they cost rows without costing scope either. ⚠️ **This number is prose about a table, which is the
...
| — | M1.33–M1.38 | no plan item: found by review after the decomposition was written — `M1.33` by the checkpoint review, `M1.34` by `M1.21`, `M1.35` by `M1.24`, `M1.36` by `M1.27`, `M1.37` and `M1.38` by `M1.28` |
