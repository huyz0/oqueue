...
| M9.16 | Rate limiting and quotas per principal | Serves NFR-12; the isolation half of multi-tenancy `M9.md`'s own framing names — a noisy tenant must not degrade another's. Doc 02 §1.7 names the wire surface if a client-visible quota API is in scope (`DescribeClientQuotas`/`AlterClientQuotas`, 48-49); this task's own acceptance criteria say whether v1 needs it on the wire or can enforce server-side only. | todo |
