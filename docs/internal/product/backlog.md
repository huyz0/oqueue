...
| M0.8 | The ten remaining crate skeletons | `buf`, `codec`, `checksum`, `index`, `store`, `coordinator`, `crypto`, `compact`, `broker` and `testkit` each exist with a `README.md` and an `AGENTS.md`; `#![forbid(unsafe_code)]` in every one but `buf`, `codec` and `checksum` (NFR-53); each depends on `oqueue-core` and on nothing else in the workspace except `broker`, which composes (NFR-52); `oqueue-testkit` is `publish = false` and appears in no runtime `[dependencies]`, only `[dev-dependencies]`, and its `README.md`/`AGENTS.md` say it holds harness and generators and **no fake** — ⚠️ `architecture.md`'s crate table still says `oqueue-testkit` holds "Fakes, generators, harness", which `contracts.md` rule 11 contradicts directly; **that cell is corrected in this commit**, because the crate's own documents and the crate map disagreeing is how the wrong convention gets re-established later; `check-readmes.sh` confirms every README's stated dependencies match its `Cargo.toml`; `cargo check --workspace` passes on both targets (NFR-40); **every crate manifest carries `lints.workspace = true`, and a gate enforces it** — ⚠️ `rust-style.md` rule 3 promises exactly that gate "once `Cargo.toml` exists, M0", nothing checks it today, and this is the row where nine more crates could each silently escape pedantic, nursery and the `unwrap_used` ban by omitting one line. `check-layering.sh` already parses every crate manifest, so it is the cheap home for the check; a crate without the line fails. ⚠️ **The same gate also rejects a `[profile]` section in a member manifest**, and this is the row where that becomes likely rather than theoretical: `build.md`'s Profiles section and `ADR-0001` both say profiles are root-only, cargo **ignores** a member-crate `[profile]` while warning on stderr and exiting 0, and this row adds ten manifests at once. A setting that is silently ignored is worse than one that is wrong, because the build succeeds and the number in the file is evidence for something that never happened. The check is one more assertion in a script already reading each manifest, which is why it is here rather than in a task of its own. ⚠️ **And the same gate asserts `overflow-checks = true` in the root manifest's `[profile.release]`** — `security.md` rule 4 does not merely imply a gate for this, it **names** one ("→ gate on the profile setting") and none exists; the failure it guards is the one that rule cites, where the debug build panics and the release build wraps, so every test in this repository passes while the shipped binary is wrong. `M0.3` added the setting and `ADR-0001` records that nothing enforces it; this is the row that closes it, and the negative-test case this row already owes covers it ⚠️ `oqueue-crypto` is absent from doc 19's ten-crate layout and exists only in `architecture.md` — dropping it is an ADR, not a silent omission. ⚠️ Before the seams, not after: `contracts.md` rule 9 puts every fake in `oqueue-core`, but M0.11's *no-op* `KeyProvider` is production code and belongs in `oqueue-crypto`, which this row creates | todo |
...
`security.md` and could only record. This one has a home: `M0.8` already gains a
...
negative-test case. The window it leaves open is `M0.4` through `M0.7`, none of
which touches a profile. Found by review, which was right that an ADR listing it
