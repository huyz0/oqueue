...
⚠️ **M1 carries 37 rows, over `sdd.md`'s cap of 20, and here is the argument
...
`M1.33`, `M1.34`, `M1.35` and `M1.36` are findings review made after this
...
threshold matcher whose backstop covers three hardcoded constants, and a gate
label pinned by accident — so they cost rows without costing scope either. ⚠️ **This number is prose about a table, which is the
...
| — | M1.33, M1.34, M1.35, M1.36 | no plan item: found by review after the decomposition was written — `M1.33` by the checkpoint review, `M1.34` by `M1.21`, `M1.35` by `M1.24`, `M1.36` by `M1.27` |
