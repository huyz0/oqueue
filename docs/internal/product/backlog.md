...
| M3.35 | The hot-path gate's excuses expire | Serves NFR-1; `performance.md` rules 18-19. `check-hot-path-bench.sh` is green on six reasons already false, and its staleness check cannot fire until someone does the work the entry excuses — so rule 19's "the list above cannot silently rot" is held by nothing. The gate fails on an entry whose stated precondition is met, measured by watching it fail; `find_batches` carries its marker and its benchmark; the five rows needing a harness that does not exist move to the deferral table. | todo |
