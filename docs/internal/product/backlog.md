...
| M3.26 | The reader's budget and failure cap are per request, not per read pass | Serves FR-13, NFR-3. `Budget` and `FetchedObjects` are built once in `read_or_park` and threaded through every pass, so four parked passes issue no more GETs than one pass may — asserted by count. A read that fetched an object and then failed to parse it is charged what it fetched and counts against the failure cap. `satisfiable_min_bytes` compares a target and a budget measured the same way. | todo |
