...
10. `M3.38`, after every behaviour change above, so the published documents
   describe what shipped rather than what was intended.
11. `M3.37`, the sweep, last of the work rows — so it also catches the
   sentences the rows above falsify.
12. **`M3.16`**, which cannot assert its zero-GET and bounded-GET claims until
...
13. **`M3.17`** last, because a closing milestone review over unfinished rows
