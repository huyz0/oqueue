...
11. `M3.38`, after every behaviour change above, so the published documents
   ...
   `Fetch`'s new null-name close**, which `M3.29`'s review found missing from
   `protocol-support.md` while the identical `ListOffsets` line sits directly
   above it.
