...
| M2.2 | ADR: codec strategy — hand-roll vs `kafka-protocol` vs codegen | Doc 10 #4, doc 02 §6.4 — decides most of this milestone's shape, before any codec code. ADR accepted, addressing version-gated field presence and flexible-versions handling; doc 10 #4 marked resolved | todo |
...
| M2.13 | Primitive codec: big-endian ints, strings, bytes, classic arrays | Serves FR-1. Round-trip property tests; every client-supplied length bounded before allocation (`security.md` rules 1-2) | todo |
...
| M2.15 | Flexible-version primitives and the per-(API, version) flexibility table | Serves FR-1, FR-2. Compact strings/arrays, tagged fields; ⚠️ no global cutover — Produce v9, Fetch v12, ApiVersions v3, SaslHandshake never. Table-driven tests per API | todo |
| M2.16 | Frame codec and request/response headers v0-v2, with the `ApiVersions` header special case | Serves FR-1. ⚠️ `ApiVersions` responses carry a v0 header even when the body is flexible. Golden bytes pin the special case; correlation-ID extraction tested | todo |
