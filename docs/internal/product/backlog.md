...
| M11.2 | `CommittedSpan` carries an optional producer identity | Serves FR-14; `ADR-0031` point 4. `oqueue-core` contract change: `producer_id: Option<ProducerId>`, `producer_epoch: Option<ProducerEpoch>`, `sequence: Option<i32>` added to `CommittedSpan`, threaded through `MetadataEntry`/`MetadataRecord` so a committed entry carries what the allocator needs to reconstruct producer state from the log on restart — the same way it already reconstructs `end_offsets`. `None` triple for an ordinary non-idempotent produce, costing nothing. Every existing caller and every `MetadataLog`/`MaterializedIndex` conformance-suite fixture updated for the new fields; not a `pub trait` method-set change, so `contract-change`'s trait-and-fakes procedure does not apply, but it is exactly the "changes an `oqueue-core` contract" case `adr.md` requires the ADR for. | todo |
