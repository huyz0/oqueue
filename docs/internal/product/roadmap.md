...
| 8 | [M4](milestones/M4.md) | Consumer groups | functional | M3, M9 | see `backlog.md` ⚠️ | `scripts/gates/m4-complete.sh` | in progress |
...
| **Nine rows M4 opened after its own decomposition and did not work** — `M4.70`, `M4.71`, `M4.72`, `M4.80`, `M4.81`, `M4.82`, `M4.84`, `M4.85`, `M4.87` | M5 (from M4) | `M1`'s eleven and `M3.41`-`M3.46` at larger scale, and the first handoff made by rule rather than by somebody noticing — `M4.73` wrote the disposition test and `M4.75`'s round applied it. ⚠️ **None of the first eight changes what a client, an operator or another tenant observes, and none is a gate that cannot fail**, which is the whole test — and `M4.87` is the exception that proves the rule is not what admitted it: it was recorded as `review.md` rule-15 *minors* on a per-commit `pass`, where the reopen test does not bind, and it is here because a minor with nowhere to go is still work somebody has to own. One of its three residues is client-observable, in the direction that answers a follower early rather than stranding it. The per-id reasons follow. `M4.70` is a prose sweep, `M4.71` is `AGENTS.md` claiming a direction `check-portability.sh` deliberately does not check, `M4.72` is a doc comment attached to the wrong function, `M4.80` is that no shell constant in the repository is covered by a completeness scan, and `M4.81` is whether `code-structure.md` rule 16's 500-line limit means non-Rust files at all — thirteen tracked ones exceed it, including two `Cargo.lock`s and three research documents, which is itself an argument about the rule's scope rather than a sweep, and is why allowlisting the six scripts is the answer that looks right and is not — `M4.82` is whether the pre-commit hook count belongs in prose as a literal at all, since `sdd.md` forbids writing a count a command derives while `m0-complete.sh` asserts two hand-copies of it and carries a third as a comment nothing reads, which is stale in the present tense inside the gate written to stop this number going stale. `M4.84` is that `M4.79`'s scoped handoff leg lives in one completion gate of nine and no document says the next one must carry it, and `M4.85` is that a boundary round which files rows can never be the last one — `milestone_commits` enumerates any commit touching more than `reviews/`, so every terminating round so far escaped by hand with a verdict-only commit the skills never name — eight milestone closings and M4's own `d2db096`, which is a terminating round inside a milestone still open. `M4.87` is three residues of `M4.83`'s own repair: a sole member's withdrawal folds over an empty roster and so leaves its own ask in place, a publish that re-takes the rounds lock can lose a member's ask to a close landing in the gap, and three sentences call the rounds-then-barrier order the crate's only lock nesting when it is one of two. ⚠️ **The last three were filed after the other six**, two by M4's fifth boundary round and one by `M4.83`'s own per-commit rounds. `M4.84` and `M4.85` do not meet the reopen test — `m4-complete.sh` carries the scoped leg today and the coverage gate fails correctly. ⚠️ **`M4.87` does meet it, and is handed on by a different route**: its second residue is client-observable, which is the test in terms, and what puts it here is that all three were recorded as `review.md` rule-15 minors on a per-commit `pass`, where the reopen test does not bind at all. Saying "it errs in the safe direction" instead would be the precedent that lets a later round hand on a major it should reopen for. ⚠️ **`M5.0` re-derives them as its own rows and marks each `dissolved` with a pointer**, `M2.0`'s and `M10.0`'s shape; `M5.md`'s plan receives them so the deferral exists in a plan rather than only here. ⚠️ **`M4.79` is why this row is written in M4 rather than left to `M5.0`**: `check-milestone-handoff.sh` cannot see a milestone until its cell reads `complete`, and the commit that flips it dissolves these rows in the same breath, so nothing would ever have failed for this row's absence |
