...
| **55 surviving mutants the full gate had never been run against** | M5 (from M4) — `M4.46` | ⚠️ **`scripts/check-mutants.sh --full` is a CI step and `M4.32` was the first time anyone ran it**: 2294 mutants, 60 missed, 1498 caught, 724 unviable, 12 timeouts, of which 55 are unargued — so that step is red. 42 of the 55 are in `oqueue-codec`, which is M2's; the rest are `oqueue-core` (6), `oqueue-broker` (4) and `oqueue-checksum` (3), and none is in code M4 wrote — the broker's four are `connection.rs` and `tls.rs`, the core's six `key_layout`, `object_meta` and `rate_governor`. ⚠️ **M5, not M12 or M15**: `M1`'s eleven rows→`M2` and `M3`'s six rows→`M10` are the precedent — debt goes to whoever opens next in execution order, not to whichever milestone the code thematically belongs to. ⚠️ Each survivor must be killed or argued individually; 55 baseline entries added to turn a gate green is what non-negotiable 2 forbids for a threshold, read across to a suppression list |
