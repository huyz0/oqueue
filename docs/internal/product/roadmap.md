...
| 14 | [M12](milestones/M12.md) | Admin API and operability | functional | M4, M9 | 20 | `scripts/gates/m12-complete.sh` | complete |
| 15 | [M13](milestones/M13.md) | Release engineering and the artifact matrix | build | M12 | 15 | `scripts/gates/m13-complete.sh` | in progress |
...
| M9 | FR-4, FR-40 (⚠️ **delivered for every API that existed at M9, and every later milestone re-asserts it for its own** — `M4.md`'s completion condition names that rule. `M4.23` found M4 met it for two of the seven APIs it added: `OffsetCommit` and `OffsetFetch` check `TopicGrants`; M12.2 discharged the five group-API remainder with `GroupGrants` and five refusal tests), FR-44, FR-45, NFR-12 |
| M4 | FR-20, FR-21 (⚠️ **its restart leg is reported, not asserted** — FR-21's own verification method is "offsets survive a full broker fleet restart", and they do not: `serve` wires `FakeGroupMetadataLog`, an in-memory `Vec`, so `M4.17` measured committed offset `5` before a restart and `-1001` after. M4 commits and serves offsets; surviving the process needs the durable engine deferred below and received by `M6.md` task 7c. `m4-complete.sh` asserts that the harness measures and reports this, never that it survived, so an auditor reading this cell against `requirements.md`'s verification method does not conclude the gate is missing an assertion and add one that must fail), FR-22 (⚠️ **deferred by `ADR-0033`, not shipped by M4** — v1 ships the classic `JoinGroup`/`SyncGroup`/`Heartbeat` protocol only, and `m4-complete.sh` deliberately asserts nothing about KIP-848. Listed here so the citation is not read as delivery; the obligation is carried by the post-v1 paragraph below. ⚠️ Do not name another requirement's id in this cell — `check-requirements-trace.sh` reads every id in it and compares the set against `M4.md`'s own `Serves:` line, so a cross-reference here reads as coverage drift), FR-40 (⚠️ **the two topic APIs were asserted at M4; the five group APIs were deferred to M12 and are now discharged by `M12.2`** with `GroupGrants`, five state-before-refusal tests, and the promoted `m4-complete.sh` source tripwire) |
| M11 | FR-14 |
...
| M6 | FR-21 (⚠️ **asserted in-crate, not yet at the client**: `committed_offsets_survive_a_restart` rebuilds a cluster from the store alone; `offset_survival.py` still restarts the binary over the in-memory store, so `m4-complete.sh`'s tripwire cannot fire — `M6.16`, handed to M7 below), FR-51, NFR-20 (⚠️ **the log's memory and snapshot grow with its age** — `M6.18`, handed on), NFR-22 (measured, not bounded — `ADR-0048`, `baselines/rto.txt`), NFR-44 |
| M7 | NFR-10, NFR-11, NFR-4 |
| M8 | FR-41 (⚠️ **the seam and both providers, against in-process simulations of each KMS API — not the vendor round trip** FR-41's verification method names: there are no cloud credentials here, and `ADR-0050` point 6 defers that evidence to M15 beside the real-S3 and real-GCS verification `M1.44` deferred there. `m8-complete.sh` says so in its own header rather than passing quietly), FR-42, FR-43 (⚠️ **deferred to M13, not shipped by M8** — `ADR-0050` point 7: the FIPS artifact needs the separate build job `M13.md` task 5 already plans, and a runtime assertion and differential test cannot live in a milestone that cannot build the artifact they assert about. Listed here so the citation is not read as delivery; the deferral row below carries it. ⚠️ Do not name another requirement's id in this cell — `check-requirements-trace.sh` compares the ids here against `M8.md`'s own `Serves:` line), NFR-14, NFR-33 |
| M12 | FR-40 (the five group APIs M4 left unmet — **discharged by M12.2** with `GroupGrants` and five cross-principal refusal tests), FR-44, FR-45, FR-50, FR-52, FR-53, NFR-12 |
| M13 | FR-43 (⚠️ **received whole from M8** by `ADR-0050` point 7 and the deferral row below: the FIPS artifact, `fips_mode_enabled()`'s runtime assertion and the FIPS↔non-FIPS differential test land together, against the build job task 5 already plans. `m13-complete.sh` needs a leg for it — its verification method is `requirements.md`'s, unchanged), NFR-40, NFR-41, NFR-42, NFR-43 |
...
|---|---|---|
| **FR-41's vendor round trip** and **FR-43 (the FIPS build)** | FR-41 → M15, FR-43 → M13 (both from M8) | `ADR-0050` points 6 and 7. FR-41: M8 builds both KMS providers behind one seam and proves portability against in-process simulations; a round trip against real AWS KMS and real GCP Cloud KMS needs credentials this project has none of, so it joins the real-S3 and real-GCS verification `M1.44` already deferred to M15. FR-43: the FIPS artifact is a separate build job with cmake and Go, which `M13.md` task 5 already owns; `fips_mode_enabled()`'s runtime assertion and the FIPS↔non-FIPS differential test land there, against the artifact they assert about. M8 keeps the algorithm agnostic at the seam, which is what the region header's `alg` field exists for. `M15.md`'s *Received from M8* and `M13.md`'s task 5 receive them |
| **Every row M7 closed over** — `M7.8`, `M7.9`, `M7.10`, `M7.12` | M15 (from M7) | M7 closes on `m7-complete.sh` with these open. ⚠️ **Three are observable, and handing them on departs from `M7.0`'s plan**, which took them because sharding multiplies them — the user's instruction that a milestone end on its gate with few tasks is the reason, stated rather than hidden: `M7.8` (group commit — every produce acknowledgement pays one object write against NFR-1), `M7.9` (the metadata log's memory and snapshot grow with its age, and a term replay reads it from zero), `M7.10` (FR-21 at the client, over MinIO). `M7.12` (where a moved topic's catalog entry lives and how an id finds its shard) goes with rebalance, which needs it first. ⚠️ **Any milestone before M15 that introduces a second shard takes `M7.12` and the four safety rows below** |
| **M7's provisional tasks `ADR-0049` does not build**, and `M6.9`, `M6.11`, `M6.12` received from M6 | M15 (from M7) | `ADR-0049` point 6: M7 builds the catalog the node looks up rather than holds, one shard behind a lookup, and the scale evidence its gate measures. Handed on: shard rebalance (`M7.md` tasks 2-3), RF-3 shard replication (9), node classes and the metadata-role boundary (7-8), the reconciliation watermark (10), the billing sweep (11), KIP-951 leader hints (13), client-default and client-ceiling documentation (14, 17), the per-connection all-topics observable (15), the parked-fetch measurement (17c), and 17a, 17b, 17d with the 404 refresh — ⚠️ **each of those four is a safety rule reachable only once a second shard or a follower cache exists**, and none does after M7, so whoever introduces either takes them in the same milestone. `M6.9`, `M6.11`, `M6.12` as M6's row above describes them. ⚠️ **M15 because nothing between M7 and it is about metadata**; any earlier milestone that introduces a second shard takes the four safety rows. `M15.md`'s *Received from M7* section receives them |
| **Every row M6 closed over** — `M6.2`, `M6.9`, `M6.11`, `M6.12`, `M6.16`, `M6.18` | M7 (from M6) — **`M7.0` re-derives `M6.2`, `M6.16`, `M6.18` as `M7.8`-`M7.10`; `M6.9`, `M6.11`, `M6.12` go on to M15** with the row below | M6 closes on `m6-complete.sh` with these open — ⚠️ **once `M6.19` has landed**, which reopened M6 and is not handed on: it is an acknowledged-data-loss path, the reopen test's plainest case. ⚠️ **Three are observable, and handing them on departs from `milestone-review`'s reopen test** — the user's instruction that a milestone end against its written exit on a small number of tasks is the reason, and the departure is stated rather than hidden: `M6.2` (no group commit, so every produce acknowledgement pays one object write against NFR-1's 500 ms p99 — M6's closing review, major), `M6.18` (the metadata log's memory and snapshot grow with the cluster's age — closing review, major), and `M6.16` (FR-21's client-level evidence: the harness restarts over the in-memory store, so the M4 tripwire cannot fire). The rest are `M6.9` (reads survive, writes pause — partly true already: an unreplayed or fenced loop refuses writes and fetches answer from the cached index), `M6.11` (the rebuild off the ack path) and `M6.12` (a follower bootstrapping from the snapshot). ⚠️ **M7 because it is next and depends on M6**: sharding multiplies every one of these by the shard count. `M7.md`'s *Received from M6* section receives them |
...
| **Group-level authorization (`GroupGrants`)** — FR-40 on the five group APIs | M12 (from M4) — **discharged by `M12.2`** | `M9` built `TopicGrants`, and `OffsetCommit`/`OffsetFetch` — the two APIs `M4` adds that name a topic — check it. ⚠️ **The other five named no topic and checked no principal at all**: `FindCoordinator`, `JoinGroup`, `SyncGroup`, `Heartbeat` and `LeaveGroup` scoped on `GroupId`, which has no principal component, so principal A could join principal B's group, be elected its leader, read every member's subscription metadata, and hold partitions B's consumers were waiting for. `M12.2` adds the separate `GroupGrants` policy, checks all five before state mutation, and makes `m4-complete.sh` assert one cross-principal refusal test per API while retaining the source tripwire. The deferral is discharged here; the two topic APIs remain covered by `TopicGrants` |
| **A durable `GroupMetadataLog`** (FR-21's own restart evidence) | M6 (from M4) — **engine discharged by `M6.6`; the harness half is `M6.16`** | ⚠️ **Amended by `M6.6` (2026-09-19)**: `bin/oqueue serve` now opens `ObjectStoreGroupMetadataLog` from the chosen store, and `committed_offsets_survive_a_restart` asserts FR-21 in-crate by rebuilding a cluster from the store alone. ⚠️ **The client-level evidence is not yet flipped**: `scripts/harness/offset_survival.py` restarts the binary over the in-memory store, where nothing survives, so it still reports the offsets lost and `m4-complete.sh`'s tripwire cannot fire. Running it over MinIO and flipping the tripwire is `M6.16`. The original rationale follows. `ADR-0035` made the group metadata log its own seam and stated that "a real engine is `M6`'s own already-scheduled work" — but that sentence lived only in the ADR: no milestone task received it and no row recorded it, so by `AGENTS.md`'s own two-places test the obligation was unscheduled. ⚠️ **`M4.17` found the gap by going to write FR-21's evidence and discovering there was none to write.** `bin/oqueue serve` wires `FakeGroupMetadataLog`, whose entries are an in-memory `Vec`, so every committed offset is lost when the process exits; measured, not inferred — committed offset `5` before a broker restart and `-1001` after. ⚠️ **So FR-21's verification method is unmet today**: `requirements.md` says "offsets survive a full broker fleet restart" and they do not. `M4.17` declines to assert it and declines to pretend — `scripts/harness/offset_survival.py` measures it every run and the harness reports a `skip` naming this row, never a pass, and **fails** if the offsets ever do survive, so this deferral cannot be discharged silently. `M6.md` task 7c receives it, beside task 7a's engine choice, which it should share rather than answer twice |
