...
measured at 2.27 s across 14 hooks — 15 once the budget gate itself joined them, 16 next, 17 at `M1.21`, 18 at `M11.9`'s `check-idempotence-enabled`, and **19 today** at the `pre-commit` stage (`M9.11`'s `check-topic-list-scope`) — `m0-complete.sh` asserts this sentence's number too, at the boundary). ⚠️ Both are **floors** — the workspace they
