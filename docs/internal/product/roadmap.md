...
| **How a `minor` review finding gets scheduled** | M2 | ⚠️ `review.md` rule 15 sends a minor to the commit body and rule 16 four lines below calls a finding living where nothing reads it one nobody will act on. M0's second half recorded 31 that way and none became a row. The fix is a **procedure** — some step that harvests commit bodies at a milestone boundary — and choosing one is a decision for whoever owns the loop, not a repair a review may make; M0's boundary review recorded it as `bcf5d6f697f2` rather than re-specifying. ⚠️ **M2 because that is the first milestone whose minors will be about protocol code rather than about prose**, and because the interim rule rule 15 now states — write the row in the *next* commit — is a workaround that removes the pressure to decide, so a date matters |
