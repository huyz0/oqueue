...
| 8 | [M4](milestones/M4.md) | Consumer groups | functional | M3, M9 | 18 | `scripts/gates/m4-complete.sh` | not started |
...
| Wiring M9's authentication/authorization/quota mechanism into `bin/oqueue serve`'s real listener | M4 (from M9) | M9's milestone-boundary review found it: `bin/oqueue`'s composition root calls `Dispatcher::new(cluster)` with none of `.tls_terminated()`, `.with_credentials(...)`, `.with_topic_grants(...)`, or `.with_quota(...)` — every M9 mechanism exists and is tested against the real `Dispatcher`, but nothing in the shipped binary reads a real TLS cert/key, credential source, topic-grant configuration, or quota configuration and passes it in. `M9.3`/`M9.4`/`M9.5`/`M9.16`'s own doc comments each name reading that configuration "`bin/oqueue`'s job, the composition root," and named it nowhere durable until now — the gap this row closes. ⚠️ **M4, not M12 or M15** — `M1`'s eleven rows→`M2` and `M3`'s six rows→`M10`'s own precedent: debt goes to whoever opens next in execution order, not to whichever milestone the work thematically resembles. Recorded as `M9.21` in `backlog.md`, `deferred` rather than `done` until `M4.0` discharges it — `M10.0`'s own precedent for `M3.41`-`M3.46`; `M4.md` task 18 receives it |
...
measured at 2.27 s across 14 hooks — 15 once the budget gate itself joined them, 16 next, 17 at `M1.21`, 18 at `M11.9`'s `check-idempotence-enabled`, 19 at `M9.11`'s `check-topic-list-scope`, and **20 today** at the `pre-commit` stage (`M9.14`'s `check-secrets`) — `m0-complete.sh` asserts this sentence's number too, at the boundary). ⚠️ Both are **floors** — the workspace they
