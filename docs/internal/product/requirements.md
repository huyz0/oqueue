...
| NFR-56 | Pre-commit suite completes within **10 s**. | `check-budget.sh` against a literal, summing each gate's own recorded wall clock | agreed — derived in `M0.16`, measured 2026-08-16 at 2.27 s across 14 hooks — 15 once this gate joined them, 16 next, 17 at `M1.21`'s conformance-matrix format check, 18 at `M11.9`'s `check-idempotence-enabled`, 19 at `M9.11`'s `check-topic-list-scope`, 20 at `M9.14`'s `check-secrets`, 21 at `M4.11`'s `check-fencing-seam`, 22 at `M4.27`'s `check-backlog-rows`, 23 at `M4.73`'s `check-milestone-handoff`, 24 at `M5.70`'s `check-wellformed`, and **25 today** (`M5.84`'s `check-milestone-exit`); ⚠️ **21 was never written down** — `M4.11` registered its hook without bumping this number, so `m0-complete.sh` would have failed at `M4`'s boundary for a hook added six tasks earlier, which is `M9.19`'s recorded precedent repeating; ⚠️ **and 23 was not either** — `M4.73` did the same, found by `M4.75`'s boundary round with `m0-complete.sh` already red, which makes three. ⚠️ **Nor was 24**: `M5.70` registered `check-wellformed` and left this cell at 23, so the tree was a hook ahead of both literals before `M5.84` added the next one — found by `M5.84`'s first round, which makes **four**, and is the strongest argument yet that the number belongs to a command rather than to a sentence. ⚠️ **The pattern is the maintenance, not the maintainer, and `M4.27` is the counter-example that shows it.** These two literals are hand-copied counts of one thing a command derives, asserted by a gate nothing on the commit path runs — so whether an increment is written down depends on the author noticing, and the three that were missed were each found by something else: `M11.9`'s, incidentally, while `M9.19` ran the gate for `M9.5`'s own change; `M4.11`'s by `M4.27`, the commit that caused the *next* increment and bumped both at once because the tree would otherwise be red; and `M4.73`'s by `M4.75`'s boundary round, four commits later. `sdd.md` forbids writing a count a command can derive and this is one; deriving it is `M4.82`'s, not this row's, because the rule that forbids it and the gate that asserts it disagree about which owns the number — and because the command this cell names, `yq '[.repos[].hooks[]] | length'`, answers **27**: it counts every hook rather than the `pre-commit`-stage ones, so discharging the deferral with it as written turns `m0-complete.sh` red. ⚠️ **That gap is the whole point and it is a moving one** — it was 25 against 23 when this sentence was written, 27 against 25 now — so the two numbers must never be quoted as though one of them were the other's evidence. The stage filter is the part nobody has written down; `m0-complete.sh` counts the `pre-commit`-stage hooks in `.pre-commit-config.yaml` and asserts this number and `roadmap.md`'s against it (`M0.25`). ⚠️ **Stage-scoped on purpose**: `check-commit-msg` and `check-tests-kept` are `commit-msg` hooks, a separate invocation whose rows never enter `check-budget.sh`'s sum, so counting *every* `- id:` line in the file would state a suite this requirement is not about. ⚠️ **That total is deliberately not written here.** It was, and it went stale the moment a `commit-msg` hook was added — with no gate to notice, since `m0-complete.sh` asserts only the `**N today**` literal above. `sdd.md` forbids writing a count a command can derive, and this is one: `yq` over every hook, which answers 27. ⚠️ **At the milestone boundary only** — that gate is in neither the hooks nor CI, so between boundaries this is as unheld as the nine claims beside it, and it stops running entirely the day a successor gate does not inherit the check. ⚠️ A **floor**: no async runtime or cloud SDK is compiled yet |
