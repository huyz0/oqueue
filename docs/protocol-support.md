...
|---|---|---|---|
| Produce | 0 | 3–13 | v0–2 removed by KIP-896. ⚠️ **A null topic name closes the connection** below v13, same as `Fetch` and `ListOffsets`. Exactly one RecordBatch (v2 magic) per partition; CRC-32C verified on ingest; base offset assigned by header rewrite. v13 addresses topics by id. `M9.12`: a topic the authenticated principal cannot see answers `TOPIC_AUTHORIZATION_FAILED` per partition, checked before the ingest rule runs — nothing is written for a refused topic. |
| Fetch | 1 | 4–17 | ⚠️ **A null topic name closes the connection** below v13, for the reason the `ListOffsets` row gives — the field is nullable on the request wire and not on the response wire. Returns whole batches from the offset asked for, resolved through the index — ⚠️ **not from the log start**, which is what `M2`'s stub had to do and what this row said until `M3.14`. Long-polls on `max_wait_ms`/`min_bytes` (`M3.20`). Fetch sessions declined (`session_id` 0 → clients full-fetch). v13+ addresses topics by id. `M9.12`: a topic the authenticated principal cannot see answers `TOPIC_AUTHORIZATION_FAILED` per partition, decided before a read and so costing no GET. |
| `ListOffsets` | 2 | 1–9 | `EARLIEST` (-2) and `LATEST` (-1) answered from the coordinator's own index, never a cache — hazard H1, whose symptom is negative consumer lag. ⚠️ A **wall-clock** timestamp is refused with `UNSUPPORTED_VERSION` (35): there is no index by time, and the nearest offset would be a silent wrong answer. ⚠️ **Not code 43**, the obvious choice — the Java consumer maps that one to a null in `offsetsForTimes`, which an application cannot tell from a truthful "nothing at or after that time". ⚠️ **A null topic name closes the connection**: the field is nullable on the request wire and not on the response wire, so there is nothing parsable to answer with. v0 is a different message (an array of offsets per partition, pre-KIP-79) and is not advertised. `M9.12`: a topic the authenticated principal cannot see answers `TOPIC_AUTHORIZATION_FAILED` for every partition named, `offset`/`timestamp` at the same unset `-1` sentinel every other refusal here uses. |
| Metadata | 3 | 0–13 | `allow_auto_topic_creation` honoured from v4 (historical always-create below). Topic ids from v10. Single node, single partition per topic. `M9.9`: an explicitly-named topic the authenticated principal cannot see (`TopicGrants`, `M9.8`) answers `TOPIC_AUTHORIZATION_FAILED` for that entry, once a credential source is configured — a no-op when none is (fail-open, `M9.7`'s own signal). `M9.10`: the null-topic-array ("every topic") case is scoped through the same index too, but silently — an unauthorized topic is omitted, never an error entry, `M9.1`'s verified Kafka finding for this specific case. |
...
| `SaslAuthenticate` | 36 | 0–2 | `M9.3`/`M9.4`. `SASL/PLAIN` (`ADR-0032`): a correctly-presented credential over a TLS-terminated connection succeeds; anything else — no TLS, wrong password, unknown principal, malformed exchange, or no credential source configured — answers `SASL_AUTHENTICATION_FAILED` (58) identically, so no failure mode is distinguishable from another. ⚠️ **No TLS listener is wired into `bin/oqueue serve` yet** (`M9.5`'s own deferred work) — until it is, every real connection is untrusted-by-default (`Dispatcher::new`'s own honest default) and `SASL/PLAIN` refuses every attempt regardless of credential correctness, the same shape `M9.3` shipped. |
